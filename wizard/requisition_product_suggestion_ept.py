...
###### Added by Viraj #######
    ...
    def prepare_main_frame(self, report_start_date, report_end_date, products, warehouses, analysis_days, forecast_stock_dict=False):
        ...
        self.prepare_main_frame_data(stock_dict, incoming_qty_dict, ads_qty_dict, products, warehouses, SortedDict(frame_period_wise))
        return report_frame

    def prepare_main_frame_data(self, stock_dict, incoming_qty_dict, ads_qty_dict, products, warehouses,frame_period_wise):
        """
        ...
        for product_id in products:
            for warehouse_id in warehouses:
                frame = {}
                if round(stock_dict.get((product_id, warehouse_id),  0.0), 2) == 0.0 and not incoming_qty_dict.get((product_id, warehouse_id), False) and round(ads_qty_dict.get((product_id, warehouse_id), 0.0), 2) == 0.0:
                    continue
    ...
    def get_recommandation_products(self):
        """
        Open recommanded report.
        :return: action with recommanded products
        """
